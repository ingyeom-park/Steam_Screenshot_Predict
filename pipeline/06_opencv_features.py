...
================================================================================
[목적]
    썸네일과 스크린샷에서 OpenCV 기반 이미지 피처를 추출해 CSV로 저장한다.
...
[출력 파일]
    data/interim/06_image_features/06_image_features.csv
        - 게임 1개당 최대 5행
        - 이 파일을 08_feature_engineering.py 에서 pivot 해 사용한다
...
OUT_DIR        = IMAGE_FEATURES_DIR
OUT_CSV        = IMAGE_FEATURES_CSV
...
def get_image_paths(appid):
    ...
    """
    paths = [("thumb", THUMB_DIR / appid / "thumb.jpg")]
    for i in range(4):
        paths.append((f"ss_{i}", SCREENSHOT_DIR / appid / f"ss_{i}.jpg"))
    return paths
